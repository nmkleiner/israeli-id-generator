export function setupIdGenerator(element) {
  element.querySelector("#generate-button").addEventListener("click", () => {
    element.querySelector("#id-input").value = generateIsraeliId();
  });

  element.querySelector("#id-input").value = generateIsraeliId();
  element.querySelector("#copy-button").addEventListener("click", () => {
    element.querySelector("#id-input").select();
    navigator.clipboard.writeText(element.querySelector("#id-input").value);
    element.querySelector("#id-input").blur();
  });
}
